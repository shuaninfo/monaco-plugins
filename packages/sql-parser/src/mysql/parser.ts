...
import { chain, many, optional, plus } from '@shuaninfo/parser'
import { createFourOperations } from '../base/four-operations'
import { comparisonOperator, dataType, logicalOperator, normalOperator, notOperator, numberSym, selectSpec, setValueList, stringOrWord, stringOrWordOrNumber, stringSym, wordSym } from '../base/parser'
...
const statement = () => {
  return chain([selectStatement, createTableStatement, insertStatement, createViewStatement, setStatement, createIndexStatement, createFunctionStatement, updateStatement])(ast => {
    return ast[0]
  })
}
...
const fromClause = () => {
  return chain('from', tableSources, optional(whereStatement), optional(groupByStatement), optional(havingStatement), (ast: any) => {
...
const joinPart = () => {
  return chain(
    ['join', 'straight_join', chain(['inner', 'cross', 'full'], 'join')(), chain(['left', 'right'], optional('outer'), 'join')(), chain('natural', optional(['left', 'right'], optional('outer')), 'join')()],
    tableSourceItem,
...
// ----------------------------------- Case -----------------------------------
const caseStatement = () => {
  return chain('case', plus(caseAlternative), optional('else', [columnField, 'null']), ['end', chain('end', 'as', wordSym)()])()
}
...
const booleanPrimary = () => {
  return chain(predicate, many(['isnull', chain([chain('is', 'not')(), 'is', 'not'], ['null', columnField])()]))(flattenAll)
}
 ...
 * */
const predicate = () => {
  return chain([chain(columnField, predicateAddonComparison)(), chain('(', predicate, ')', predicateAddonComparison)()])()
}

const predicateAddonComparison = () => {
  return chain(optional([chain(comparisonOperator, columnField)(), chain('sounds', 'like', columnField)(), isOrNotExpression]), optional(['or', predicate]))()
}
...
const isOrNotExpression = () => {
  return chain(optional('is'), optional('not'), [chain('in', '(', [selectStatement, fieldList], ')')(), chain('between', field, 'and', predicate)(), chain('like', field, optional('escape', field))(), chain('regexp', field)(), 'null'])()
}
