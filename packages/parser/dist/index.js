...
var createParser = (root, lexer, options) => {
  return (text, cursorIndex = null) => {
    ...
    let error = null;
    if (!success) {
      const suggestions = uniqBy((lastMatchUnderShortestRestToken ? findNextMatchNodes(lastMatchUnderShortestRestToken.matchNode, parser) : findNextMatchNodes(parser.rootChainNode, parser)).map((each) => {
